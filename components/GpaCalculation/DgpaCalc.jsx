...
const DgpaCalc = (props) => {
  ...
  props.data.marksPercentage.map((marks, index) => {
    options2.push(marks.gradePoint);
  });

  const options3 = [];
  props.data.marksPercentage.map((marks, index) => {
    options3.push(marks.letterGrade);
  });
  ...
  props.data.marksPercentage.map((marks, index) => {
    if (index == 0) {
      numerical.push(`${marks.start}% or above`);
    } else if (index == props.data.marksPercentage.length - 1) {
      numerical.push(`less than ${parseFloat(marks.end) + 1}%`);
    } else {
      numerical.push(
        `${marks.start}% to less than ${parseFloat(marks.end) + 1}%`
      );
    }
  });
  ...
  const returnGpa = (number) => {
    ...
    props.data.marksPercentage.map((marks, index) => {
      if (
        number >= parseFloat(marks.start) &&
        number < parseFloat(marks.end) + 1
      ) {
        track = index;
      }
    });
  ...
  const deductionMark = parseFloat(props.data.deductionMark);

  const threshold = parseFloat(options2[options2.length - 2]);
  ...
  const inputCheckerMarkDown = (inputVals) => {
    ...
    values.map((value, index) => {
      ...
      if (value.type == "(retake)" && parseFloat(value.gpa) > threshold) {
        const mod = {
          name: value.name,
          totalCredits: value.totalCredits,
          gpa: (parseFloat(value.gpa) - deductionMark).toFixed(2),
          marks: value.marks,
          letter: value.letter,
          type: value.type,
        };
  ...
  const CalcHandler = () => {
    ...
    inputValues.map((term) => {
      ...
      if (term.type == "(regular)") {
        ...
        credits = parseFloat(term.totalCredits);
        gpa = parseFloat(term.gpa) - deductionMark;
        if (gpa < threshold) {
          gpa = gpa + deductionMark;
        }
      }
  ...
  return (
    <div className="w-full h-full flex flex-col justify-center items-center">
      {att && (
        <AttendanceCalc data={props.data.attendancePercentage}></AttendanceCalc>
      )}
      <button
      ...
      <form className="w-full flex flex-col justify-center items-center">
        <div className="w-full flex flex-col justify-center items-center mt-8">
          {inputValues.map((value, index) => (
            <div
              key={index}
              className="w-full flex flex-col justify-center items-center mb-8 appear-div"
            >
